...
lmm.df <- lmm.df %>%
  ...
  dplyr::select(-text_ID, -fb_type, -FreqC1, -FreqC2, -fb_count_NA)
...
# 1.3.7. Apply Bonferroni correction

p_values_FreqC <- c(prop_FreqC_X2, prop_FreqC_X3, prop_FreqC_X4)
p_values_mtld <- c(prop_mtld_X2, prop_mtld_X3, prop_mtld_X4)

num_comparisons <- length(p_values_FreqC)

p_values_FreqC_corrected <- p.adjust(p_values_FreqC, method = "bonferroni")
p_values_mtld_corrected <- p.adjust(p_values_mtld, method = "bonferroni")
...
preferences_and_performance <- preferences_and_performance[, c("participant_ID",
                                                               ...
                                                               "TextNum",
                                                               "error_rate", "error_rate_per_type",
...
set.seed(17)
up.preferences_and_performance <- upSample(preferences_and_performance[,-59],
...
preferences_and_success <- preferences_and_performance %>%
  rowwise() %>%
...
preferences_and_success <- preferences_and_success %>%
  rowwise() %>%
...
preferences_and_success <- preferences_and_success %>%
  dplyr::select(participant_ID, fb_type, error_type,
                clarity, usefulness, liking)

preferences_and_success <- distinct(preferences_and_success)


# 3.1.2. Combine with predicted probabilities

habits_beliefs_data <- merge(new_X, preferences_and_success, by = c("error_type", "fb_type"))
habits_beliefs_data <- merge(habits_beliefs_data, preferences_and_performance, by = c("error_type", "fb_type"))
habits_beliefs_data <- habits_beliefs_data %>%
  dplyr::select(participant_ID = participant_ID.x,
                error_type,
                fb_type,
                `Probability of error fixing` = predicted_prob,
...
cols.correlation <- habits_beliefs_data[, c(4:9)]
...
num_tests <- choose(ncol(corr.matrix), 2)
...
# Check if each correlation is significant after Bonferroni correction
significant_results <- which(p_values < alpha_adjusted)
```
